...
export default function ComensalesMesa() {
  ...
  const imprimirComanda = () => {
    ...
    if (!pdfWindow) { alert('Permite ventanas emergentes para ver el PDF'); return }
  ...
  const cerrarCuenta = async () => {
    ...
    const pdfWindow = window.open('', '_blank')
    if (!pdfWindow) { alert('Permite ventanas emergentes para ver el ticket'); return }
    ...
    <div style={styles.page}>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.list}>
          ...
          {libre && (
            <p style={styles.empty}>Mesa libre — agrega un comensal para iniciar</p>
          )}
        </div>

        <div style={styles.addRow}>
          <input style={styles.input} placeholder="Nuevo comensal" value={newName}
            onChange={e => setNewName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addComensal()} />
          <button style={styles.addBtn} onClick={addComensal}>+</button>
        </div>
...
const styles = {
  ...
  input: {
    flex: 1, padding: '12px 14px', borderRadius: 8, border: '1px solid #ddd',
